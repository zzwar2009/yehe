...
function getVisitorListByNickname(queryData) {
    ...
    if (queryData.name) {
        queryData.where = {
            supplierASName: `like[${queryData.name}]`, // 供应商名称模糊查询
        };
        delete queryData.name;

        // supplierASCode
    } else if (queryData.supplierASCode) {
        queryData.where = {
            supplierASCode: queryData.supplierASCode, // 精确查找
        };
        delete queryData.name;
    } else {
        queryData.where = {};
    ...
    return queryStations(queryData);
}
//点击空白收起
function close(e) {
...
export default class StaffListSelect extends Component {
            ...
            if (this.state.value != nextProps.value) {
                ...
                if (value) {
                    let queryData = {
                        page: 1,
                        size: 10000,
                        supplierASCode: value,
                    };
                    getVisitorListByNickname(queryData).then(function(res) {
                        const { code, result } = res;
                        if (code == 200) {
                            for (let i = 0; i < result.length; i++) {
                                let { supplierASCode, supplierASName, id } = result[i];
                                if (supplierASCode == value) {
                                    var state = Object.assign({}, that.state);
                                    state.nickname = supplierASName;
                                    state.value = supplierASCode;
                                    that.setState(state);
                                    break;
                                }
                            }
    ...
    onSelect(item) {
        ...
        //选择某个结果
        var state = Object.assign({}, this.state);
        let supplierASCode = item.supplierASCode;
        let data = state.data;
        state.opened = false;
        state.value = supplierASCode;
        state.data = [];
        for (let i = 0; i < data.length; i++) {
            let { supplierASCode, supplierASName, id } = data[i];
            if (supplierASCode == state.value) {
                state.nickname = supplierASName;
                break;
            }
        }
        ...
        if (this.props.onChange) {
            this.props.onChange(item.supplierASCode, item);
        }
    }
    search(e) {
        //模糊搜索昵称
    ...
    render() {
        ...
        lis = data.map(function(item, i) {
            let { supplierASName, supplierASCode, id } = item;
            return (
                <li
                    key={i}
                    className="ellipsis"
